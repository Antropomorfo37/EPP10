# RUNBOOK — Operational Closure Sequence (SECURE VERSION)

**IMPORTANTE**: Esta versión utiliza variables de entorno en lugar de rutas hardcodeadas para mayor seguridad y portabilidad.
...
## Configuración Inicial — Environment Variables

Antes de ejecutar cualquier paso, establece estas variables de entorno:

```bash
# Opción 1: En la sesión actual
export EPP10_PROJECT_ROOT="$HOME/EPP10"
export EPP10_DATA_DIR="${EPP10_PROJECT_ROOT}/data"
export EPP10_CACHE_DIR="${EPP10_PROJECT_ROOT}/cache_bootstrap_B2000"
export EPP10_CACHE_PIPELINE="${EPP10_PROJECT_ROOT}/cache_bootstrap_pipeline"

# Opción 2: Persistente (añadir a ~/.bashrc o ~/.zshrc)
echo 'export EPP10_PROJECT_ROOT="$HOME/EPP10"' >> ~/.bashrc
echo 'export EPP10_DATA_DIR="${EPP10_PROJECT_ROOT}/data"' >> ~/.bashrc
echo 'export EPP10_CACHE_DIR="${EPP10_PROJECT_ROOT}/cache_bootstrap_B2000"' >> ~/.bashrc
echo 'export EPP10_CACHE_PIPELINE="${EPP10_PROJECT_ROOT}/cache_bootstrap_pipeline"' >> ~/.bashrc
source ~/.bashrc
```

**Verificación**:
```bash
echo "Proyecto: ${EPP10_PROJECT_ROOT}"
echo "Datos: ${EPP10_DATA_DIR}"
echo "Cache B2000: ${EPP10_CACHE_DIR}"
```
...
ls "${EPP10_CACHE_DIR}/" | grep rep_ | wc -l
...
for f in hormones_long_tidy.csv cohort_normalization_map.csv \
  ...
  [ -f "${EPP10_PROJECT_ROOT}/$f" ] && echo "OK  $f" || echo "MISSING  $f"
done

# Verify figures + verification docs
ls "${EPP10_PROJECT_ROOT}"/figures/Figure[1-3]*.pdf
ls "${EPP10_PROJECT_ROOT}"/verification/compliance_tick_sheet.md
ls "${EPP10_PROJECT_ROOT}"/verification/Verification_Appendix_S3.md
```
...
```bash
cd "${EPP10_PROJECT_ROOT}"
Rscript -e '
project_root <- Sys.getenv("EPP10_PROJECT_ROOT", ".")
...
writeLines(capture.output(sessionInfo()),
           file.path(project_root, "verification/sessionInfo.txt"))
'
```

Verification:
```bash
[ -f "${EPP10_PROJECT_ROOT}/renv.lock" ] && echo "renv.lock OK"
wc -l "${EPP10_PROJECT_ROOT}/verification/sessionInfo.txt"
```
...
Rscript -e '
project_root <- Sys.getenv("EPP10_PROJECT_ROOT", ".")
r <- readRDS(file.path(project_root, "bootstrap_B2000_results.rds"))
...
- Figures 1-3 (PDF + PNG)
- Security: Uses environment variables for path configuration
...
```bash
cd "${EPP10_PROJECT_ROOT}/manuscript"
...
git commit -m "B=2000 snapshot: $(date +%Y-%m-%d) progress $(ls "${EPP10_CACHE_DIR}/" | grep rep_ | wc -l)/2000"
...
- [ ] Environment variables properly configured for reproducibility
...
- Using environment variables instead of hardcoded paths
- Reducing exposure of local filesystem structure
- Making scripts portable across different machines
- Supporting `.env` file for CI/CD integration
- Preventing accidental exposure of personal paths in documentation

⚠️ **Still TODO:**
- Add `.env.example` template file
- Review R scripts for internal hardcoded paths
- Implement GitHub Secrets for CI/CD workflows

---

**This runbook is self-contained.** Execute sequentially; each step's verification block confirms success before proceeding. Steps 1-4 take ~15 min total. Steps 5-7 (DOI + manuscript compile) take additional 30-45 min depending on internet latency.
